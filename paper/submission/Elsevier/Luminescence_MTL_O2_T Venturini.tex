\documentclass[preprint,12pt,a4]{elsarticle}
%\documentclass[9pt,twocolumn,twoside,pdftex]{osa-article}
...
\author[toelt, zhaw]{Francesca Venturini}
...
\begin{keyword}
Spectroscopy \sep Fluorescence \sep Luminescence \sep Neural networks \sep Remote sensing \sep Sensors
...
\right)
\end{equation}
where $w_i$ are the 50 values of the angular modulation frequency of the excitation light (see Sec. \ref{Experimental}). \hl{This choice of dividing by 90 have been made since, during testing, have been observed that such a normalisation delivers the best results and avoid problems like divergence of the neural network model.}
...
To evaluate the performance of the sensor, different metrics were analyzed. These are discussed in the next sections. \hl{The measured dataset $S$ have been divided in two parts: one containing 80\% of randomly chosen observations (indicated with $S_{train}$), and one containing the remaining 20\% of the data (indicated with $S_{test}$). The results shown in the following sections have been obtained by measuring the different metrics on the $S_{test}$ dataset.}
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=13.5 cm]{KDE_results_all.eps}
