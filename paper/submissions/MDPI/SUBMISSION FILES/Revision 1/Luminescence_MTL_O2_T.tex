...
In this work, we describe a new type of learning sensor with parallel inference which overcomes all these difficulties. 
With this sensor we show how to generate automatically and autonomously a very large dataset of measurements and how these are used for the training of the neural-network-based signal processing. This allows the elimination of any approximate empirical model and the use of a machine learning algorithm to do inference. Furthermore, in this work we demonstrate how the sensors learns from the cross-interference to extract multiple parameters from a single set of optical measurements without any {\sl a priori} mathematical model, and with unprecedented accuracy. Finally, we propose a completely new metric to characterize the performance of neural-network-based sensors, the Error Limited Accuracy. In general, the methods described here are not limited to oxygen and temperature sensing. They can be similarly applied for the sensing with multiple luminophores, whenever the underlying mathematical model is not known or too complex.}
% Keywords
