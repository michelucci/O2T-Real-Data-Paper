...
In this work, we describe a new type of learning sensor with parallel inference. 
{\bf This sensor, for the first time, is capable of generating automatically and autonomously a very large dataset of measurements that can be used to train the neural network based signal processing algorithm with real data that is specific to the hardware itself. This allowed the elimination of the typically used empirical mathematical model and the use of a machine learning algorithm to do inference.}
In this work we demonstrate how it is possible for such a  sensor to extract multiple parameters from a single set of optical measurements without the need for any {\sl a priori} mathematical model, and with unprecedented accuracy. We also propose a completely new metric to characterise the performance of neural network based sensors, the Error Limited Accuracy. The described sensor is not limited to oxygen and temperature sensing. It can be used for the sensing with multiple luminophores, whenever the underlying mathematical model is not known or too complex.}
% Keywords
...
%% OLD PARAGRAPH
%Sensors based on this principle must rely on approximated empirical models to parametrize the dependence of the measured sensing quantity (e.g., luminescence intensity or decay time) on influencing factors. Among these, the temperature is the factor with the strongest influence since both the luminescence and the quenching phenomena are strongly temperature-dependent. 
%% END OF OLD PARAGRAPH


Therefore, in any optical oxygen sensor, the temperature must be continuously monitored, most frequently with a separate sensor, and used to correct the calculated oxygen concentration \cite{Li2015}. This task can be difficult in practical implementation and may become a significant source of error. % in sensors based on luminescence sensing. 
...
%This new approach is based on multi-task learning (MTL) neural network architectures. These are characterized by common hidden layers, whose output is then the input of multiple branches of task-specific hidden layers. 
...
%Luminescence Quenching for Oxygen Determination}
\label{Theory}
